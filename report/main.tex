...
\def\csupervisor{Oskar Janson (\url{oskar.janson@cinnober.com})\\}
...
\textbf{Cryptocurrency} - A currency backed not by centralized authorities but by mathematical evidence and clever mechanisms.\\
...
\textbf{Hash} - A hash is a mathematical way of producing a one-way \enquote{fingerprints} of any data, easy to check but nearly impossible to reverse.\\
\textbf{Nonce} - A temporary value, most often a number. Used to give the hash of a data structure a unique outcome. The value fills no purpose other than altering the hash. \\
\textbf{Mining} - In the context of cryptocurrency refers to looking for a valid hash in a proof-of-work system. Most often by putting random numbers in the nonce field until a valid hash is found.\\
\textbf{On-chain} - Something that is on-chain (for example a transaction) has been included in the blockchain and can be safely assumed to be immutable, in other words can not be changed.\\
\textbf{Off-chain} - Something that is not on the blockchain. Often meaning that it can be altered in some way.\\
...
\textbf{Alice and Bob} - Alice and Bob are two fictional people used in examples. Sometimes referred to as just A and B\\
...
\textbf{Merkle tree and merkle root} - A merkle tree is a tree structure created from hashes of a list of elements. The root
is the singular node formed at the top of the tree. The tree makes it easier to check if an element was part of the original list without \\
...
\input{atomic_swaps/swaps}

\input{implementation/implementation}

\input{proposal/result_and_proposal}

\input{future/future_and_research}
